...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-2xl border border-[var(--border-soft)] bg-[var(--surface)]">
      <Link href={`/product/${product.slug}`} className="relative aspect-square overflow-hidden bg-[var(--bg-1)]">
        <Image src={product.imagePath} alt={`${product.name} vial artwork`} fill className="object-cover" />
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <button
          ...
          className="mt-5 rounded-full bg-[var(--accent)] px-4 py-3 text-sm font-semibold text-[var(--bg-0)] transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-40"
        >
